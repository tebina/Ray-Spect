...
file = open('input_files/example_aes_sbox_netlist', 'r')
sample = file.read()
...
for rectangle in rectangles:
    region_count += 1
    def_obj = DefParser("input_files/example_aes_sbox.def", rectangle[0], rectangle[1])
    fetched_instances = def_obj.region_fetch()
    netlist_obj = pp(parsed_netlist, fetched_instances, "vthadd", 0.8)
    netlist_obj.generate_netlist("generated_netlists/regional_netlist" + str(region_count))

    del def_obj
    del netlist_obj

# def_obj = DefParser("example_aes_sbox_netlist/sbox.def", (0, 0), (400, 400))
# fetched_instances = def_obj.region_fetch()
# netlist_obj = pp(parsed_netlist, fetched_instances, "vthadd", 0.8)
